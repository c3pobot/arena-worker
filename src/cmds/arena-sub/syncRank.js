...
module.exports = async(obj = {}, oldData, pObj, sId, chId)=>{
  ...
  if(!pObj) pObj = { allyCode: +obj.allyCode, playerId: obj.playerId, name: obj.name, notify: { status: 0, poNotify: 0, timeBeforePO: 24, climb: 0, method: 'log', type: 0 } }
  ...
  let charPOhour = GetPOHour(obj.poOffSet, 'char'), shipPOhour = GetPOHour(obj.poOffSet, 'ship'), oldCharRank = oldData.char.currentRank, oldShipRank = oldData.ship.currentRank
  let charObj, shipObj
  if(pObj){
    charObj = {
      name: obj.name,
  ...
  if(dataChange > 0){
    ...
    await mongo.set('rankCache', {_id: obj.playerId}, oldData)
    if(oldData.history){
      delete oldData.history
      await mongo.rep('rankCache', {_id: obj.playerId}, oldData)
    }
  }
}
